...
type AdminHotelPartnerDeleteDialogProps = Readonly<{
...
export function AdminHotelPartnerDeleteDialog({
  target,
  isSubmitting,
  onCancel,
  onConfirm,
}: AdminHotelPartnerDeleteDialogProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-600">
          {isDeactivate
            ? t("deleteDialog.description", { name: target.partner.name })
            : t("permanentDeleteDialog.description", { name: target.partner.name })}
        </p>
        {target.partner.bookingCount > 0 ? (
          <p className="mt-2 text-sm font-medium text-amber-800">
            {t("deleteDialog.bookingWarning", { count: target.partner.bookingCount })}
          </p>
        ) : null}
